...
class Nav2Point(Node):
    def __init__(self):
        ...
        self.declare_parameter('avoid_radius', 1.20)
        self.declare_parameter('emergency_distance', 0.45)
        self.declare_parameter('repulsion_gain', 0.85)
        ...
        self.repulsion_gain = float(
            self.get_parameter('repulsion_gain').value)
        ...
        self.sub_scan = self.create_subscription(
            LaserScan, self.get_parameter('scan_topic').value, self.cb_scan, qos)
    ...
    def cb_scan(self, msg: LaserScan):
        ...
        for distance in msg.ranges:
            if math.isfinite(distance) and msg.range_min <= distance <= self.avoid_radius:
                points.append((float(distance), angle))
            angle += float(msg.angle_increment)
        self.scan_points=points

    def apply_local_avoidance(self, vx, vy, wz):
        if not self.scan_points:
            return vx,vy,wz
        rep_x=rep_y=0.0
        front=left=right=math.inf
        for distance,angle in self.scan_points:
            weight=(1.0/distance - 1.0/self.avoid_radius) / max(distance,0.1)
            rep_x -= weight*math.cos(angle)
            rep_y -= weight*math.sin(angle)
            if abs(angle) < math.radians(38):
                front=min(front,distance)
            if math.radians(15) < angle < math.radians(100):
                left=min(left,distance)
            if -math.radians(100) < angle < -math.radians(15):
                right=min(right,distance)

        norm=math.hypot(rep_x,rep_y)
        if norm > 1e-6:
            rep_x/=norm
            rep_y/=norm
            strength=min(1.0, norm/8.0)
            vx += self.repulsion_gain*strength*rep_x
            vy += self.repulsion_gain*strength*rep_y

        if front < self.avoid_radius:
            scale=max(0.0,min(1.0,
                (front-self.emergency_distance) /
                (self.avoid_radius-self.emergency_distance)))
            vx=min(vx,self.vx_lim*scale)
        if front < self.emergency_distance:
            vx=min(vx,0.0)
            direction=1.0 if left > right else -1.0
            vy=direction*min(self.vy_lim,0.35)
            wz=direction*min(self.wz_lim,0.35)

        return (
            max(-self.vx_lim,min(self.vx_lim,vx)),
            max(-self.vy_lim,min(self.vy_lim,vy)),
            max(-self.wz_lim,min(self.wz_lim,wz)),
        )
    ...
    def loop(self):
        try:
            ...
            vx_b,vy_b,wz=self.apply_local_avoidance(vx_b,vy_b,wz)
